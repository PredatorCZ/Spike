...
inline void _DecodeBC1Block(const char *data, char *obuffer, muint32 w,
                            muint32 h, muint32 width, uint32 pixeloffset = 3) {
  mreal32x3 color1, color2;
...
inline void _DecodeBC1BlockA(const char *data, char *obuffer, muint32 w,
                             muint32 h, muint32 width, uint32 pixeloffset = 4) {
  mreal32x3 color0;
  mreal32x3 color1;
  DecodeRGB565Block(data, color0);
  uint16 cl0 = reinterpret_cast<uint16 &>(*data);
  data += 2;
  DecodeRGB565Block(data, color1);
  uint16 cl1 = reinterpret_cast<uint16 &>(*data);
  data += 2;
  h *= 4;
  w *= 4;
  width *= 4;
  ...
  for (int row = 0; row < 4; row++) {
    ...
    auto &v0 = reinterpret_cast<muint8x4 &>(
        *(obuffer + ((h + row) * width + w) * pixeloffset)) = colors[d1];
    v0.w = usealpha && d1 == 3 ? 0 : 0xff;
    auto &v1 = reinterpret_cast<muint8x4 &>(
        *(obuffer + ((h + row) * width + w + 1) * pixeloffset)) = colors[d2];
    v1.w = usealpha && d2 == 3 ? 0 : 0xff;
    auto &v2 = reinterpret_cast<muint8x4 &>(
        *(obuffer + ((h + row) * width + w + 2) * pixeloffset)) = colors[d3];
    v2.w = usealpha && d3 == 3 ? 0 : 0xff;
    auto &v3 = reinterpret_cast<muint8x4 &>(
        *(obuffer + ((h + row) * width + w + 3) * pixeloffset)) = colors[d4];
    v3.w = usealpha && d4 == 3 ? 0 : 0xff;
  }
}

inline void _DecodeBC2Block(const char *data, char *obuffer, muint32 w,
                            muint32 h, muint32 width, uint32 pixeloffset = 4) {
  h *= 4;
  w *= 4;
  width *= 4;
...
inline void _DecodeBC4Block(const char *data, char *obuffer, muint32 w,
                            muint32 h, muint32 width, uint32 pixeloffset = 1) {
  h *= 4;
  w *= 4;
  width *= 4;

  muint16 alpha[8]{*reinterpret_cast<uint8 *>(data++),
