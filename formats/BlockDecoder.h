...
ES_INLINE void DecodeRGB565Block(const char *data, Vector &color)
{
...
ES_INLINE void ComputeBC5Blue(char *buffer, int size)
{
	for (int p = 0; p < size; p += 3)
	{
		const Vector2 RG((reinterpret_cast<UCVector2*>(buffer + p + 1)->Convert<float>() * (2.0f / UCHAR_MAX)) - 1.f);
